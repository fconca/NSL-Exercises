...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if(_particle(i).getspin() == -1){
      prob = 1. / (1. + exp(-_beta * delta_E));
    }
    else if(_particle(i).getspin() == 1){
      prob = 1. / (1. + exp(_beta * delta_E));
    }
    if(_rnd.Rannyu() < prob) _particle(i).flip();
  } else {           // M(RT)^2
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutp << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_PE:" << setw(20) << "PE_AVE:" << setw(20) << "ERROR:" << endl;
        coutp.close();
        _nprop++;
        ...
        coutk << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_KE:" << setw(20) << "KE_AVE:" << setw(20) << "ERROR:" << endl;
        coutk.close();
        _nprop++;
        ...
        coutt << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_TE:" << setw(20) << "TE_AVE:" << setw(20) << "ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutte << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_T:" << setw(20) << "T_AVE:" << setw(20) << "ERROR:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        ...
        coutpr << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_P:" << setw(20) << "P_AVE:" << setw(20) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_M:" << setw(20) << "M_AVE:" << setw(20) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_CV:" << setw(20) << "CV_AVE:" << setw(20) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << setw(7) << "#BLOCK:" << setw(20) << "ACTUAL_X:" << setw(20) << "X_AVE:" << setw(20) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pressure){
    virial = _rho * (2.0/3.0) * kenergy_temp + 16.0/_volume * virial/double(_npart);
  ...
  if (_measure_magnet){
    ...
    _measurement(_index_magnet) = s / _npart;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
  ...
  for (int i=0; i<_npart; i++)
    s += double(_particle(i).getspin());
  _measurement(_index_chi) = _beta * s * s / _npart;
  }
...
void System :: averages(int blk){
  ...
  if(_measure_cv)
    _average(_index_cv) = _npart * _beta * _beta * (_average(_index_cv) - _average(_index_tenergy)*_average(_index_tenergy));
  _global_av  += _average;
  ...
  if (_measure_penergy){
    ...
    coutf << setw(7) << blk 
          << setw(20) << scientific << average
          << setw(20) << scientific << sum_average/double(blk)
          << setw(20) << scientific << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
...
void System :: set_temperature(double new_temp){
  if (new_temp < 0){
    cerr << "PROBLEM: temperature must be a positive value" << endl;
    exit(1);
  }
  _temp = new_temp;
  _beta = 1.0 / new_temp;
...
void System :: copy_output_in_input(){

  static bool delete_config_file = true;
  if (delete_config_file){
    remove("../OUTPUT/CONFIG/control_config.spin");
    delete_config_file = false;
  }

  ifstream inc("../INPUT/CONFIG/config.spin");
  ofstream outc("../OUTPUT/CONFIG/control_config.spin", ios::app);

  outc << setw(20) << "TEMPERATURE: " << _temp << endl
         << setw(20) << "INPUT: ";

  int spin;
  if (inc.is_open() && outc.is_open()) {
    while (inc >> spin) {
      outc << setw(4) << spin;
    }
  } 
  ...
  inc.close();
  ifstream inf("../OUTPUT/CONFIG/config.spin");

  outc << endl << setw(20) << "OUTPUT: ";

  if (inf.is_open() && outc.is_open()) {
    while (inf >> spin) {
      outc << setw(4) << spin;
    }
  }
  ...
  inf.clear();
  inf.seekg(0, ios::beg);
  ofstream outf("../INPUT/CONFIG/config.spin");
...
void System :: read_last_row(const string& filename, double& average, double& error) {
  ifstream in(filename);

  if (in.is_open()) {
    int num_rows = get_nbl(); 
    int num_columns = 4; 
...
void System :: print_output(){
  static bool print_legend = true;
  static bool delete_config_file = true;
  ...
  if (_measure_tenergy){
    if (delete_config_file) 
      remove("../OUTPUT/temp_total_energy.dat");

    in.open("../OUTPUT/total_energy.dat");
    out.open("../OUTPUT/temp_total_energy.dat", ios::app);

    if (in.is_open() && out.is_open()){
      double average; double error;
      read_last_row("../OUTPUT/total_energy.dat", average, error);
      if (print_legend) out << setw(12) << "TEMPERATURE:" << setw(20) << "TOTAL_ENERGY" << setw(20) << "ERROR:" << endl; 
      out << setw(12) << _temp << setw(20) << average << setw(20) << error << endl;
      out.close();
      in.close();
    }
  }
  ...
  if (_measure_magnet){
    if (delete_config_file) 
      remove("../OUTPUT/temp_magnetization.dat");

    in.open("../OUTPUT/magnetization.dat");
    out.open("../OUTPUT/temp_magnetization.dat", ios::app);

    if (in.is_open() && out.is_open()){
      double average; double error;
      read_last_row("../OUTPUT/magnetization.dat", average, error);
      if (print_legend) out << setw(12) << "TEMPERATURE:" << setw(20) << "MAGNETIZATION" << setw(20) << "ERROR:" << endl; 
      out << setw(12) << _temp << setw(20) << average << setw(20) << error << endl;
      out.close();
      in.close();
    }
  ...
  if (_measure_cv){
    if (delete_config_file) 
      remove("../OUTPUT/temp_specific_heat.dat");

    in.open("../OUTPUT/specific_heat.dat");
    out.open("../OUTPUT/temp_specific_heat.dat", ios::app);

    if (in.is_open() && out.is_open()){
      double average; double error;
      read_last_row("../OUTPUT/specific_heat.dat", average, error);
      if (print_legend) out << setw(12) << "TEMPERATURE:" << setw(20) << "SPECIFIC_HEAT" << setw(20) << "ERROR:" << endl; 
      out << setw(12) << _temp << setw(20) << average << setw(20) << error << endl;
      out.close();
      in.close();
    }
  ...
  if (_measure_chi){
    if (delete_config_file) 
      remove("../OUTPUT/temp_susceptibility.dat");

    in.open("../OUTPUT/susceptibility.dat");
    out.open("../OUTPUT/temp_susceptibility.dat", ios::app);

    if (in.is_open() && out.is_open()){
      double average; double error;
      read_last_row("../OUTPUT/susceptibility.dat", average, error);
      if (print_legend) out << setw(12) << "TEMPERATURE:" << setw(20) << "SUSCEPTIBILITY" << setw(20) << "ERROR:" << endl; 
      out << setw(12) << _temp << setw(20) << average << setw(20) << error << endl;
      out.close();
      in.close();
    }
