  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8.0/9.0 * M_PI * _rho * (1.0 / pow(_r_cut,9) - 3.0 / pow(_r_cut,3)); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 16.0/9.0 * M_PI * _rho * (2.0 / pow(_r_cut,9) - 3.0/pow(_r_cut,3)); // TO BE FIXED IN EXERCISE 7
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << setw(20) << "# DISTANCE:" << setw(20) << "AVE_GOFR:" << setw(20) << "ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr && dr < _halfside.min()) {
          int index_bin = int(dr / _bin_size);
          _measurement(_index_gofr + index_bin) += 2.0;
        }
        if(dr < _r_cut){
  ...
  if (_measure_pressure){
    virial = _ptail + _rho * (2.0/3.0) * kenergy_temp + 16.0/_volume * virial/double(_npart);
  ...
  if (_measure_gofr){
    for (int i=0; i<_n_bins; i++){
      _measurement(_index_gofr + i) = _measurement(_index_gofr + i) / (4.0/3.0 * M_PI * _rho * _npart * (pow((i+1)*_bin_size,3) - pow(i*_bin_size,3)));
    }
  }
...
void System :: averages(int blk){
  ...
  if (_measure_gofr && blk == _nblocks){
    ...
    for (int i=0; i<_n_bins; i++){
      ...
      coutf << setw(20) << scientific << _bin_size * (i+0.5)
          << setw(20) << scientific << sum_average/double(blk)
          << setw(20) << scientific << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();   
