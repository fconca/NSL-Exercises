...
double Random :: Expo(double lambda) {
   // This function generates a random number from an exponential distribution with given exponential factor lambda
   double x = Rannyu();
   return -1/lambda*log(1-x);
}

double Random :: Cauchy(double mean, double gamma) {
   // This function generates a random number from a Cauchy-Lorentz distribution with given HWHM gamma and mean
   double x = Rannyu();
   return gamma*tan(M_PI*(x-1/2))+mean;
}
